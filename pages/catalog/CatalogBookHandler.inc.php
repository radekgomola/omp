...
class CatalogBookHandler extends Handler {
    ...
    function book($args, $request) {
        ...
        if ($ompPaymentManager->isConfigured()) {
            $availableFiles = array_filter(
                    $submissionFileDao->getLatestRevisions($publishedMonograph->getId()), create_function('$a', 'return $a->getViewable() && $a->getDirectSalesPrice() !== null && $a->getAssocType() == ASSOC_TYPE_PUBLICATION_FORMAT;')
            );
    ...
    function chapter($args, $request) {
        ...
        $chapter = $chapterDao->getChapter($args[1]);
        if($chapter == NULL){
            $this->book($args,$request);
        }
        ...
        $this->setupTemplate($request, $publishedMonograph);
        $templateMgr->assign('chapter', $chapter);
        $templateMgr->assign('monograph', $publishedMonograph);
        $templateMgr->assign('publicationFormats', $publicationFormats);

        // Display
        $templateMgr->display('frontend/pages/chapter.tpl');
    }
    ...
    function download($args, $request, $view = false, $flipbook = false) {
        ...
        $queuedPayment = $ompPaymentManager->createQueuedPayment(
                $press->getId(), PAYMENT_TYPE_PURCHASE_FILE, $user->getId(), $fileIdAndRevision, $submissionFile->getDirectSalesPrice(), $press->getSetting('currency')
        );

        $ompPaymentManager->displayPaymentForm(
                $ompPaymentManager->queuePayment($queuedPayment), $queuedPayment
        );
    }
