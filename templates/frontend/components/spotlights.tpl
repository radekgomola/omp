...
<div class="cmp_spotlights">
    <div class="crossroad-std">
        ...
        <ul {*class="spotlights"*} class="grid">
            {foreach name="spotlights" from=$spotlights item=spotlight}
...
{*                    {assign var="authors" value=$item->getAuthorString(false, ",<br />","; ", true)|strip_unsafe_html}*}
                ...
                {if $spotlight->getLocalizedDescription()}
                    {assign var=description value=$spotlight->getLocalizedDescription()|truncate:350|strip_unsafe_html}
                {else}
                    {if $assocType == $smarty.const.SPOTLIGHT_TYPE_SERIES}
                        {assign var=description value=$item->getLocalizedDescription()|truncate:350|strip_unsafe_html}
                    {elseif $assocType == $smarty.const.SPOTLIGHT_TYPE_BOOK}
                        {assign var=description value=$item->getLocalizedAbstract()|truncate:350|strip_unsafe_html}
                    {/if}
                {/if}

                <li {*class="spotlight_{$smarty.foreach.spotlights.iteration}{if $smarty.foreach.spotlights.iteration == 1} current{/if}"*} class="grid__cell size--m--2-4 size--l--3-12">
