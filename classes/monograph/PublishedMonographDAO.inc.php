...
define('ORDERBY_DATE_MODIFIED', 'dateModified');
...
class PublishedMonographDAO extends MonographDAO {
    ...
    function getPressFeatures($pressId, $rangeInfo = null) {
        $params = array_merge(
                $this->getFetchColumns(), array(ASSOC_TYPE_PRESS, (int) $pressId)
        );
        $result = $this->retrieveRange(
                'SELECT	ps.*,
				s.*,
			...
			ORDER BY f.seq, ps.date_published', $params, $rangeInfo
        );
    ...
    function getById($monographId, $pressId = null, $metadataApprovedOnly = true) {
        ...
        $result = $this->retrieve(
			...
			WHERE	s.submission_id = ?
                ...
                . ($metadataApprovedOnly ? ' AND ps.date_published IS NOT NULL' : ''), $params
        );
    ...
    function getByIdsCategory($monographIds, $pressId = null, $categoryId = null, $metadataApprovedOnly = true) {
        $result = $this->retrieve(
			...
			WHERE	sc.category_id = ?
				AND ps.date_published IS NOT NULL', array((int) $categoryId)
        );
    ...
    function getByIdsSeries($monographIds, $pressId = null, $serieId = null, $metadataApprovedOnly = true) {
        $result = $this->retrieve(
			...
			WHERE	s.series_id = ?
				AND ps.date_published IS NOT NULL', array((int) $serieId)
        );
    ...
    function getByBestId($monographId, $pressId) {
        $publishedMonograph = null;
        if ($monographId != '')
            $publishedMonograph = $this->getByPubId('publisher-id', $monographId, $pressId);
        if (!isset($publishedMonograph) && ctype_digit("$monographId"))
            $publishedMonograph = $this->getById((int) $monographId, $pressId);
        return $publishedMonograph;
    }
    ...
    function insertObject($publishedMonograph) {
        ...
        $this->update(
				...
				VALUES
				(?, %s, ?, ?, ?, ?, ?, ?)', $this->datetimeToDB($publishedMonograph->getDatePublished())), array(
    ...
    function deleteById($monographId) {
        $this->update(
                'DELETE FROM published_submissions WHERE submission_id = ?', (int) $monographId
        );
    }
    ...
    function updateObject($publishedMonograph) {
        $this->update(
				...
				SET	date_published = %s,
					...
					cover_image = ?
				WHERE	submission_id = ?', $this->datetimeToDB($publishedMonograph->getDatePublished())), array(
            $publishedMonograph->getAudience(),
            $publishedMonograph->getAudienceRangeQualifier(),
            $publishedMonograph->getAudienceRangeFrom(),
            $publishedMonograph->getAudienceRangeTo(),
...
//                                        . 'LEFT JOIN submission_chapter_authors scha ON (sch.chapter_id = scha.chapter_id) '
				...
				' . ($searchText !== null ? '
					LEFT JOIN authors a ON s.submission_id = a.submission_id
                                        ...
                                        LEFT JOIN publication_format_doi pfs ON (pfs.publication_format_id = pf.publication_format_id)
				...
				LEFT JOIN new_releases nr ON (nr.submission_id = s.submission_id AND nr.assoc_type = ? AND nr.assoc_id = ?)
			WHERE	ps.date_published IS NOT NULL AND s.context_id = ?
				' . ($searchText !== null ? ' AND (st.setting_value LIKE ? OR a.first_name LIKE ? OR a.last_name LIKE ? OR pfs.setting_value LIKE ? )' : '') . '
				' . ($assocType == ASSOC_TYPE_CATEGORY ? ' AND (c.category_id IS NOT NULL OR sc.category_id IS NOT NULL)' : '') . '
				' . ($assocType == ASSOC_TYPE_SERIES ? ' AND se.series_id = ' . $assocId : '') . '
				' . ($featuredOnly ? ' AND (f.assoc_type = ? AND f.assoc_id = ?)' : '') . '
				' . ($newReleasedOnly ? ' AND (nr.assoc_type = ? AND nr.assoc_id = ?)' : '') . '
    ...
    private function _getByAssocCount($pressId, $assocType, $assocId, $obor = null, $rok_vydani = null, $jazyk = null, $fakulta = null) {
        ...
        $result = $this->retrieve(
			...
			WHERE	ps.date_published IS NOT NULL AND s.context_id = ?
                ...
                ' . ($jazyk == "VYBER-MS" ? 'AND msl.iso IS NOT NULL GROUP BY msl.iso' : '' ) . '
